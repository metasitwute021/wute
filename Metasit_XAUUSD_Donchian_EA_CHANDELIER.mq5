...
#property version   "2.20c"
#property strict
//+------------------------------------------------------------------+
//|  *** CHANDELIER EDITION (experiment, separate file) ***          |
//|  Adds a classic Chuck-LeBeau Chandelier Exit as an ALTERNATIVE   |
//|  trailing engine (InpUseChandelier).                             |
//|    Long  SL = HighestHigh(N) - ATR(N) * mult                     |
//|    Short SL = LowestLow(N)   + ATR(N) * mult                     |
//|  The stop "hangs" off the recent extreme by an ATR distance, so  |
//|  it widens when volatility rises (lets price breathe / pull back)|
//|  and tightens as the market calms. When InpUseChandelier=false   |
//|  the EA behaves exactly like the standard multi-layer build.     |
...
input group "🕯️  CHANDELIER EXIT (experiment)"
input bool     InpUseChandelier      = true;       // 🧪 ON = replace multi-layer trail with Chandelier Exit (Chuck LeBeau)
input int      InpChandPeriod        = 22;         // ↳ ATR period for Chandelier (classic = 22)
input double   InpChandMult          = 3.0;        // ↳ ATR multiplier (classic = 3.0; higher = looser, lets it run more)
input int      InpChandLookback      = 22;         // ↳ highest-high / lowest-low lookback bars (classic = 22)
...
int hChand    = INVALID_HANDLE;   // Chandelier Exit ATR (trailing TF)
...
int OnInit()
{
   ...
   hChand    = iATR(_Symbol, InpTrailTF, InpChandPeriod);
...
void OnDeinit(const int reason)
{
   ...
   if(hChand!=INVALID_HANDLE)    IndicatorRelease(hChand);
...
// Chandelier Exit (Chuck LeBeau): stop hangs off the recent extreme by an ATR distance.
//   Long  SL = HighestHigh(N) - ATR(N) * mult
//   Short SL = LowestLow(N)   + ATR(N) * mult
// Returns 0 if data not ready. ModifySL still only ever tightens, so it never loosens.
double ComputeChandelierSL(const bool isBuy)
{
   double atr;
   if(!BufVal(hChand, 0, 0, atr) || atr <= 0.0) return 0.0;
   double dist = atr * InpChandMult;

   if(isBuy)
   {
      int idx = iHighest(_Symbol, InpTrailTF, MODE_HIGH, InpChandLookback, 1);
      if(idx < 0) return 0.0;
      double hh = iHigh(_Symbol, InpTrailTF, idx);
      if(hh <= 0.0) return 0.0;
      return hh - dist;
   }
   else
   {
      int idx = iLowest(_Symbol, InpTrailTF, MODE_LOW, InpChandLookback, 1);
      if(idx < 0) return 0.0;
      double ll = iLow(_Symbol, InpTrailTF, idx);
      if(ll <= 0.0) return 0.0;
      return ll + dist;
   }
}
...
{
   // 🕯️ Chandelier mode replaces the multi-layer trail entirely.
   if(InpUseChandelier)
      return ComputeChandelierSL(isBuy);
