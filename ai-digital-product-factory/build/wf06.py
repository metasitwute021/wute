...
JS_BUILD_SQL = r"""
// Normalise the record, then build both dialects up front. Which one actually
// runs is decided by the DB_TYPE switch below.
const VALID_OPS = ['run_start', 'product_upsert', 'status_update', 'error_log'];

const rec = $input.first().json || {};
const op = VALID_OPS.includes(rec.op) ? rec.op : 'product_upsert';
...
// $1 .. $14 - the order is shared by both dialects.
const values = [
  rec.run_id,
  rec.factory || null,
...
// --- SQLite: no parameter binding through the CLI, so escape by hand. ------
const q = (value) => {
  if (value === null || value === undefined) return 'NULL';
  return "'" + String(value).replace(/'/g, "''").replace(/\r/g, '').replace(/^ADPFSQL$/gm, 'ADPF_SQL') + "'";
};
...
const sqliteSql = [
  ...
  "SELECT 'ok' AS result, (SELECT id FROM adpf_products WHERE run_id = " + q(values[0]) + ') AS product_id;',
].join('\n');
...
return [{
  json: {
    ...
    run_id: rec.run_id,
    status,
    db_type: String($env.DB_TYPE || 'postgres').toLowerCase(),
    pg_params: values,
...
def build() -> Workflow:
    ...
    wf.add("Postgres: Ensure Schema", T_POSTGRES, pos(3, 0),
           {"operation": "executeQuery", "query": DDL_POSTGRES, "options": {}},
           notes="Idempotent DDL so a fresh database bootstraps itself on the first run.",
           **RETRY)
