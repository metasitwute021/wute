//+------------------------------------------------------------------+
//|                            XAUUSD_Donchian_SL_EA_WideTrail.mq5    |
...
//|                  trailing layers. InpTrailWidest picks WIDEST     |
//|                  stop (more room, fewer bounces) instead of the   |
//|                  tightest. Default = widest (this variant).       |
...
//+------------------------------------------------------------------+
#property copyright "XAUUSD Donchian EA (Donchian-SL, wide-trail variant)"
...
input bool     InpTrailWidest        = true;       // Trailing stop choice: true = WIDEST (more room, fewer bounces) / false = tightest (original)
...
double ComputeTrailingSL(const bool isBuy, const double price)
{
   ...
   //   InpTrailWidest = false -> "most protective" = tightest (hugs price, original)
   //   InpTrailWidest = true  -> "most room"       = widest valid stop. Fewer
   //          premature stop-outs / rides the trend longer. ModifySL still only
   //          ever tightens, so the SL never loosens once set.
   double best = 0.0;
   bool   have = false;
   for(int i=0;i<c;i++)
   {
