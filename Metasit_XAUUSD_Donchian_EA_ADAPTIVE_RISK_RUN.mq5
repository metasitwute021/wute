...
#property version   "2.20a"
#property strict
//+------------------------------------------------------------------+
//|  *** ADAPTIVE RISK + LET-RUN edition (experiment) ***            |
//|  (InpLetWinnersRun defaults TRUE here = runner mode + adaptive)   |
//|  Risk per trade steps DOWN on losses and back UP only when safe: |
//|    - Loss -> step down one rung: 1.0% -> 0.75% -> 0.5% (floor)    |
//|    - Win  -> step up one rung ONLY if recovered to >= a safe      |
//|              distance from the trailing death line (peak - 4%);   |
//|              otherwise HOLD the current rung.                     |
//|  Once at 0.5% it stays there through further losses (you only     |
//|  ever fail while risking the smallest size). State (rung + peak)  |
//|  is stored in GlobalVariables so a restart does not reset it.     |
//|  InpAdaptiveRisk=false -> behaves exactly like the base EA.       |
//+------------------------------------------------------------------+
...
input group "🎚️  ADAPTIVE RISK LADDER (experiment)"
input bool     InpAdaptiveRisk       = true;       // 🧪 ON = risk steps DOWN on losses, back UP only when safe (overrides fixed Risk%)
input double   InpRiskRungHigh       = 1.0;        // Top rung risk % (start / full)
input double   InpRiskRungMid        = 0.75;       // Middle rung risk %
input double   InpRiskRungLow        = 0.5;        // Bottom rung risk % (floor — stays here through further losses)
input double   InpTrailDD_Percent    = 4.0;        // Account trailing max-loss % (death line = peak − this)
input double   InpRecoverBufferPct   = 2.5;        // Step UP a rung only when equity is ≥ this % away from the death line
...
int OnInit()
{
   ...
   LoadAdaptiveState();   // restore risk-ladder rung + peak (survives restart)
...
//==================================================================
//  🎚️ ADAPTIVE RISK LADDER
//    gRiskLevel: 2=High(1.0%) 1=Mid(0.75%) 0=Low(0.5%). Starts High.
//==================================================================
int gRiskLevel = 2;

string AdaptLvlKey()  { return "MtsAdaptLvl_"  + IntegerToString(InpMagic); }
string AdaptPeakKey() { return "MtsAdaptPeak_" + IntegerToString(InpMagic); }

double RungPct(const int lvl)
{
   if(lvl <= 0) return InpRiskRungLow;
   if(lvl == 1) return InpRiskRungMid;
   return InpRiskRungHigh;
}

// Risk % the NEXT trade will actually use.
double CurrentRiskPercent()
{
   return InpAdaptiveRisk ? RungPct(gRiskLevel) : InpRiskPercent;
}

// Restore ladder state on start so a VPS/terminal restart does not reset it.
void LoadAdaptiveState()
{
   gRiskLevel = 2;
   if(!InpAdaptiveRisk) return;
   if(GlobalVariableCheck(AdaptLvlKey()))
   {
      int lvl = (int)MathRound(GlobalVariableGet(AdaptLvlKey()));
      if(lvl >= 0 && lvl <= 2) gRiskLevel = lvl;
   }
   if(GlobalVariableCheck(AdaptPeakKey()))
   {
      double pk = GlobalVariableGet(AdaptPeakKey());
      if(pk > gPeakEquity) gPeakEquity = pk;   // keep the running peak (for the death line)
   }
}
...
{
   if(!InpAdaptiveRisk) return;
   GlobalVariableSet(AdaptLvlKey(),  gRiskLevel);
   GlobalVariableSet(AdaptPeakKey(), gPeakEquity);
}

// Called after a position FULLY closes (net P/L of the whole position).
void UpdateRiskLadder(const double tradeNetPL)
{
   if(!InpAdaptiveRisk) return;
   int prev = gRiskLevel;
   ...
   if(tradeNetPL < 0.0)
   {
      if(gRiskLevel > 0) gRiskLevel--;                  // LOSS -> step down (floor at Low)
   }
   else if(tradeNetPL > 0.0)
   {
      double equity     = AccountInfoDouble(ACCOUNT_EQUITY);
      double ddFromPeak = (gPeakEquity > 0.0) ? (gPeakEquity - equity) / gPeakEquity * 100.0 : 0.0;
      double distToDeath = InpTrailDD_Percent - ddFromPeak;   // % room left to the trailing death line
      if(distToDeath >= InpRecoverBufferPct && gRiskLevel < 2)
         gRiskLevel++;                                  // WIN + far enough from death -> step up
   }
   // scratch (net == 0): hold

   if(gRiskLevel != prev)
      Notify(StringFormat("🎚️ Adaptive risk %s → %.2f%% (was %.2f%%)",
                          (gRiskLevel > prev ? "UP ⬆️" : "DOWN ⬇️"),
                          RungPct(gRiskLevel), RungPct(prev)));
   SaveAdaptiveState();
}
...
double CalcLotSize(const ENUM_ORDER_TYPE type, const double entry, const double sl)
{
   ...
   double riskMoney = balance * (CurrentRiskPercent() / 100.0);
   double riskDist  = MathAbs(entry - sl);
   ...
   PrintFormat("SIZING bal=%.2f risk%%=%.2f riskMoney=%.2f | dist=%.2f lossPerLot=%.2f | raw=%.5f K(%s)=%.0f conf=%.2f -> final=%.2f | lossAtFinal=%.2f (=%.2f%% of bal)",
               balance, CurrentRiskPercent(), riskMoney, riskDist, lossPerLot,
...
void AccumulateAndMaybeFinalize(const ulong posId, const double profit)
{
   ...
   if(!PositionSelectByTicket(posId))
   {
      ...
      UpdateRiskLadder(total);                      // 🎚️ adjust adaptive risk rung (win up / loss down)
