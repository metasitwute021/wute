...
//+------------------------------------------------------------------+
#property copyright "XAUUSD Donchian EA"
...
input bool     InpCentAccount        = false;      // Account is a Cent account
...
input int      InpInitSL_ATRPeriod   = 14;         // Initial SL ATR period (entry TF)
input double   InpInitSL_ATRMult     = 2.0;        // Initial SL = ATR x mult
...
int hATRinit  = INVALID_HANDLE;   // entry-TF ATR for initial SL
int hATR1     = INVALID_HANDLE;   // trailing layer 1
int hATR2     = INVALID_HANDLE;   // trailing layer 2 (strong move)
int hATR3     = INVALID_HANDLE;   // trailing layer 3 (mini strong move)
...
int OnInit()
{
   ...
   hATRinit = iATR(_Symbol, InpEntryTF, InpInitSL_ATRPeriod);
   ...
   if(hMA==INVALID_HANDLE || hADX==INVALID_HANDLE || hATRinit==INVALID_HANDLE ||
...
void OnDeinit(const int reason)
{
   ...
   if(hATRinit!=INVALID_HANDLE) IndicatorRelease(hATRinit);
...
void TryEnter()
{
   ...
   double atrInit;
   if(!BufVal(hATRinit, 0, 1, atrInit) || atrInit<=0.0) return;
   double riskDist = atrInit * InpInitSL_ATRMult;

   bool buySignal  = (close1 > upper) && (close1 > ma);
   bool sellSignal = (close1 < lower) && (close1 < ma);

   if(buySignal)
      OpenTrade(ORDER_TYPE_BUY, riskDist);
   else if(sellSignal)
      OpenTrade(ORDER_TYPE_SELL, riskDist);
}

void OpenTrade(const ENUM_ORDER_TYPE type, const double riskDist)
{
   double price = (type==ORDER_TYPE_BUY) ? SymbolInfoDouble(_Symbol, SYMBOL_ASK)
                                         : SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double sl    = (type==ORDER_TYPE_BUY) ? price - riskDist
                                         : price + riskDist;
   ...
   bool ok = (type==ORDER_TYPE_BUY) ? trade.Buy(lots, _Symbol, price, sl, 0.0, "Donchian")
                                    : trade.Sell(lots, _Symbol, price, sl, 0.0, "Donchian");
   if(!ok)
   {
...
void UpdateConfidence()
{
   ...
   int total = ArraySize(gClosedProfit);
   if(total < 3)   // not enough history -> neutral
   {
