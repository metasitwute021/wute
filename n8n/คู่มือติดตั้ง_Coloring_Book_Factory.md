...
**ผลงานต่อ 1 ชุด (6 หน้ารวมปก, กระดาษ A4 แนวนอน):**
...
| `ชื่อเล่ม.pdf` (ไฟล์แนบ) | รวมเล่ม 6 หน้า: **ปกสี 1 + หน้าระบายสีขาวดำ 5** A4 แนวนอน มีกรอบเว้นขอบกระดาษ พร้อมพิมพ์/ขาย |
...
ถ้าไม่พิมพ์ไอเดีย ทุกครั้งที่รัน AI จะ**สุ่มธีมใหม่เอง**จากคลัง 22 ธีม (อวกาศ, พ่อมดแม่มด, สัตว์ป่า, มุมเมือง, ใต้ทะเล, ไดโนเสาร์, สวนแฟรี่, ฟาร์ม, ร้านขนมหวาน, ฮาโลวีนน่ารัก, หมู่บ้านคริสต์มาส, โรงงานหุ่นยนต์, คาเฟ่ชาญี่ปุ่น, อ่าวโจรสลัด, ซาฟารี, เบเกอรี่, แคมป์ปิ้งในป่า, เพื่อนหิมะขั้วโลก, ร้านดอกไม้, วงดนตรีจิ๋ว, ทะเลวันหยุด, ห้องสมุดเวทมนตร์) พร้อมคิดตัวละคร (สลับกันทั้งสัตว์น่ารัก / คนตัวจิ๋ว / คนคู่กับสัตว์) ชื่อเล่ม และฉากเองทั้งหมด
...
2. **Agent 2 — Prompt Artist**: เขียน prompt ภาพ ล็อกสไตล์ตายตัว (เส้นดำหนา ขาวดำล้วน กรอบเว้นขอบ **ของไม่เยอะ เส้นไม่รก มีพื้นที่ว่างให้ระบายสบาย** / ปกเป็นภาพสี+ชื่อเล่ม)
3. **Agent 3 — Art Director QA**: ตรวจแก้ทุก prompt ก่อนจ่ายเงินสร้างภาพ (กันคำสี กันฉากซ้ำ กันของไม่เหมาะกับเด็ก และ**นับของในฉาก ถ้าเกินโควตาจะตัดออกให้**)
...
| รายการ | ราคา |
|---|---|
| gpt-image-1 ขนาด 1536x1024 quality `high` (คมชัดสุด) × 6 ภาพ | ~$1.50 (~55 บาท) |
| gpt-4.1-mini (agents 3 ตัว) | ~$0.01 |
| **รวม** | **~$1.51/ชุด** — รันทุกวันทั้งเดือน ~$45 |

ประหยัดขึ้น: เปลี่ยน `image_quality` ในโหนด **Config** เป็น `medium` (~$0.43/ชุด คมชัดปานกลาง) หรือ `low` (~$0.11/ชุด เหมาะทดสอบระบบ)
...
| ภาพรก เส้นเยอะ ของล้นหน้า | โหนด **Agent 3** → ลดตัวเลขในข้อ 2 ของ checklist (ตอนนี้ในห้อง 6 ชิ้น นอกห้อง 3 ชิ้น) ให้น้อยลงอีก |
| ภาพโล่งเกินไป | ทำกลับกัน — เพิ่มตัวเลขในข้อ 2 ของ Agent 3 และแก้คำว่า `only a few large clear objects` ในบล็อกสไตล์ Agent 2 |
