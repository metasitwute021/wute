...
//+------------------------------------------------------------------+
#property copyright "Metasit XAUUSD Donchian EA - WEEKEND-GUARD TEST build"
#property version   "2.05-weekend"
#property description "TEST build: optional weekend guard (close all + no Friday-late entries). Backtest to compare DD/return."
#property strict

#define EA_VERSION "2.05-weekend"
...
input group "=== Weekend guard (TEST) ==="
input bool     InpNoWeekendHold      = false;      // [WEEKEND TEST] close all before weekend + block Friday-late entries (avoid Mon gap)
input int      InpWeekendCloseHour   = 20;         // [WEEKEND TEST] Friday SERVER hour to close all / stop new entries (0-23)
...
datetime gWeekendClosedDay = 0;   // [WEEKEND TEST] which Friday we already flat-closed, avoid repeat
...
void OnTick()
{
   ...
   CheckWeekendClose();     // [WEEKEND TEST] flat all before the weekend (Trump gap guard)
   SyncPositionState();
...
//==================================================================
//  [WEEKEND TEST] Weekend guard: flat all + block new entries on
//  Friday from InpWeekendCloseHour onward (avoids Mon-open gap risk,
//  e.g. "Trump weekend" geopolitical headlines). Off by default.
//==================================================================
bool IsWeekendBlockTime()
{
   ...
   MqlDateTime dt;
   TimeToStruct(TimeCurrent(), dt);
   // day_of_week: 0=Sun .. 5=Fri .. 6=Sat ; market is closed Sat/Sun anyway
...
void CheckWeekendClose()
{
   ...
   {
      CloseAllMyPositions();
      Notify(StringFormat("🌅 WEEKEND GUARD: closed all before weekend (Fri %02d:00 server) - avoid Mon gap",
                          dt.hour));
   }
...
bool TradingAllowed()
{
   ...
   if(IsWeekendBlockTime()) return false;   // [WEEKEND TEST] no new entries Friday-late
