...
//|  - Sizing      : risk 1% via OrderCalcProfit (accurate on Cent)  |
...
//|                  re-entry cooldown after a close (anti-whipsaw)  |
...
//|                                                                  |
//|  *** EA023 = the PROFITABLE base (wide Donchian SL + patient     |
//|      locked trailing) MERGED with EA022's safety features: ***    |
//|        + re-entry cooldown (InpReentryCooldownBars)              |
//|        + OrderCalcProfit lot sizing (accurate on Cent)          |
//|        + min-lot over-risk warning                               |
//|      SL + trailing logic kept EXACTLY as the profitable base.    |
//+------------------------------------------------------------------+
#property copyright "XAUUSD Donchian EA (Donchian-SL + EA022 safety merge)"
...
void OnTick()
{
   ...
   // trading again. Skipping a bar also forces a FRESH signal recalculation
   // on the next bar instead of blindly re-entering the same spot.
...
double CalcLotSize(const ENUM_ORDER_TYPE type, const double entry, const double sl)
{
   ...
   // Loss (account currency) for 1.0 lot if SL is hit. OrderCalcProfit()
   // accounts for contract size + cent-account currency automatically, so it
   // gives the TRUE risk on any account type (more robust than tickValue math).
   ...
   double k         = GetLotCoefficient();                 // broker/account profile (1.0 default)
   ...
   PrintFormat("SIZING bal=%.2f risk%%=%.2f riskMoney=%.2f | dist=%.2f lossPerLot=%.2f | raw=%.5f K=%.0f conf=%.2f -> final=%.2f | lossAtFinal=%.2f (=%.2f%% of bal)",
               balance, InpRiskPercent, riskMoney, riskDist, lossPerLot,
               rawLots, k, conf, lotsFinal, lossAtFinal, riskPctReal);
